...
public class SetICDLabelTraductionPL {
  ...
  private static String labelsFileNamePl = PropertiesUtil.getProperties("labelsFileNamePl");
  private static OWLDataFactory fact = null;
  static InputStream input = null;
  
  public static void main(final String[] args) throws Exception {
    
    input = new FileInputStream(owlFileNameDE);
    ChargeMapping.chargeCSVTraductionPlFileToList(labelsFileNamePl);
    ...
    forEach(ax -> {
      if(ChargeMapping.listTraductionPl.containsKey(((OWLAnnotationAssertionAxiom) ax).getValue().toString().split("@")[0].substring(1, ((OWLAnnotationAssertionAxiom) ax).getValue().toString().split("@")[0].length()-1).toLowerCase().replaceAll("[^a-zA-Z0-9_-]", ""))) {    
        final OWLAnnotation labelAnno = fact.getOWLAnnotation(((OWLAnnotationAssertionAxiom) ax).getProperty(),fact.getOWLLiteral(ChargeMapping.listTraductionPl.get(((OWLAnnotationAssertionAxiom) ax).getValue().toString().split("@")[0].substring(1, ((OWLAnnotationAssertionAxiom) ax).getValue().toString().split("@")[0].length()-1).toLowerCase().replaceAll("[^a-zA-Z0-9_-]", "")), "pl"));
    ...
    final OutputStream fileoutputstream = new FileOutputStream(owlFileNamePL);
